...
export default function JobSearch() {
    ...
    const handleSearch = async (e) => {
        ...
        try {
            const res = await jobAPI.search(filters);
            const data = Array.isArray(res.data) ? res.data : res.data?.content ?? [];
            setJobs(data);
        ...
        <div>
            ...
            <form onSubmit={handleSearch} className="bg-white dark:bg-stone-800 border-[3px] border-stone-900 dark:border-stone-700 shadow-[8px_8px_0_#1c1917] dark:shadow-[8px_8px_0_#000] p-6 sm:p-8 mb-8 rounded-none">
                ...
                <div className="flex flex-col sm:flex-row gap-4 mt-6">
                    <button
                        type="submit"
                        className="neo-btn flex-1 sm:flex-initial bg-gradient-to-r from-orange-500 to-rose-500 px-8 py-3 dark:border-stone-700 text-white"
                    >
                        Search
                    </button>
                    <button
                        ...
                        >
                            <div className="flex justify-between items-start">
                                <div className="flex-1 flex gap-4">
                                    <div className="w-12 h-12 flex-shrink-0 bg-orange-100 dark:bg-stone-900 border-[2px] border-stone-900 dark:border-stone-700 flex items-center justify-center font-black text-2xl text-orange-600 dark:text-orange-500 shadow-[2px_2px_0_#1c1917] dark:shadow-[2px_2px_0_#000]">
                                        {job.employer?.firstName ? job.employer.firstName.charAt(0).toUpperCase() : '💼'}
                                    </div>
                                    <div>
